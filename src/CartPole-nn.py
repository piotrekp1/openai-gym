...
def take_action(bot, observation, action_space, eps=0.05, diagnostic=False):
    try:
        if random.random() > eps:
            ...
            x_batch = np.array([np.append(i, observation) for i in range(action_space.n)])
...
num_games_per_episode = 2000
data = []
for i_episode in range(num_episodes):
    ep_data = np.empty(shape=(0, env.observation_space.shape[0] + 2))
    for i_game in range(num_games_per_episode):
        ...
        ep_samples = 80
        game_data = np.empty(shape=(0, env.observation_space.shape[0] + 2))
        # Start Episode
        if i_game % 1000 == 0:
            print(i_game)
        ...
        for t in range(500):
            ...
            row = np.append((reward, action), observation)
    ...
    epoch_num = 15 if i_episode == 1 else 1
    bot.fit(train_data[:, 1:], train_data[:, 0], epochs=max(15 - (i_episode * 3), 2))
env.close()
